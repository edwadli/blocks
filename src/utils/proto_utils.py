...
def protobuf_to_dict(pb, type_callable_map=_TYPE_CALLABLE_MAP, use_enum_labels=False):
    ...
    if extensions:
        result_dict[_EXTENSION_CONTAINER] = extensions
    return result_dict
...
def dict_to_protobuf(pb_klass_or_instance, values, type_callable_map=_REVERSE_TYPE_CALLABLE_MAP, strict=True):
...
def _get_field_mapping(pb, dict_value, strict):
    ...
    for ext_num, ext_val in dict_value.get(_EXTENSION_CONTAINER, {}).items():
        try:
            ext_num = int(ext_num)
        except ValueError:
...
def MergeDictToProto(values, message):
